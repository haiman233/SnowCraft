...
class SnowGolemAssembler(
    ...
    private fun constructMenu(preset: BlockMenuPreset) {
        ...
        preset.addItem(
            7,
            CustomItem(MaterialData(Material.IRON_BLOCK), "§7雪槽", "", "§r这个槽位用于放置雪")
        ) { _: Player?, _: Int, _: ItemStack?, _: ClickAction? -> false }
        preset.addItem(
            13,
    ...
    val snowGolemPumpkinSlots: IntArray
        ...
        addItemHandler(object : BlockTicker() {
            override fun tick(b: Block, sf: SlimefunItem, data: Config) {
                ...
                if (lifetime % 60 == 0) {
                    ...
                    for (slot in snowGolemPumpkinSlots) {
                        if (SlimefunManager.isItemSimiliar(
                                BlockStorage.getInventory(b).getItemInSlot(slot), ItemStack(
                                    Material.PUMPKIN
                                ), true, SlimefunManager.DataType.ALWAYS
                    ...
                    if (ironblock > 1 && pumpkin > 0) {
                        ...
                        Bukkit.getScheduler().runTask(SlimefunStartup.instance) {
                            val snowman = b.world.spawnEntity(
                                Location(b.world, b.x + 0.5, b.y + offset, b.z + 0.5),
                                EntityType.SNOWMAN
                            ) as Snowman
                            val snowmanmaxhealth = snowman.getAttribute(Attribute.GENERIC_MAX_HEALTH)
                            snowmanmaxhealth.addModifier(
                                AttributeModifier(
                                    "MAX_HEALTH",
                                    20.0,
        ...
        object : BlockMenuPreset(name, inventoryTitle) {
            ...
            override fun newInstance(menu: BlockMenu, b: Block) {
                try {
                    ...
                    menu.addMenuClickHandler(
                        31
                    ) { _: Player?, _: Int, _: ItemStack?, arg3: ClickAction ->
                        val offset1: Double = DoubleHandler.fixDouble(
                            BlockStorage.getLocationInfo(b.location, "offset")
                                .toDouble() + if (arg3.isRightClicked) -0.1f else 0.1f
                        )
...
internal object SnowGolemAssemblerListener {
    @SubscribeEvent
    fun e(e: EntityDeathEvent) {
        if (e.entity.type == EntityType.SNOWMAN) {
            val snowman = e.entity as Snowman
            val snowmanmaxhealth = snowman.getAttribute(Attribute.GENERIC_MAX_HEALTH)
            if (snowmanmaxhealth.value == 24.0) {
                val randomInt = RandomUtils.nextInt(1, 101)
                if (randomInt <= 5) {
                    e.entity.world.dropItem(e.entity.location, Items.SNOW_GOLEM_HEART)
                }
            }
        }
    }
